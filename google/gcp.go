...
// func New(conf Config, injectionKey string, opts ...option.ClientOption) (pubsub pubsub.Pubsub, err error) {
...
// func (gcp *GCP) Send(providedContext context.Context, topics []string, message Message) error {
...
// func (gcp *GCP) Listener(providedContext context.Context, subscriptionID string) (pubsub.Listener, error) {
// 	ctx, cancel := gcp.ctx(providedContext)
//
...
// 	messageType, messageVersion string, initMessage func() Message,
// ) (chMessage chan Message) {
// 	chMessage = make(chan Message, listener.channelBufferSize)
...
// func (listener *GCPListener) Unmatched() (chUnmatched chan *Unmatch) {
// 	if listener.chUnMatched == nil {
// 		listener.chUnMatched = make(chan *Unmatch, listener.channelBufferSize)
// 	}
